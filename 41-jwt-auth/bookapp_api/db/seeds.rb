@book_data = [
  {
    "id": 1,
    "title": "Soul Kiss",
    "author": "Shay Youngblood",
    "img": "https://images-na.ssl-images-amazon.com/images/I/31AYTZWPCPL._SX309_BO1,204,203,200_.jpg",
    "read": false
  },
  {
    "id": 2,
    "title": "Children of Blood & Bone",
    "author": "Tomi Adeyemi",
    "img": "https://images-na.ssl-images-amazon.com/images/I/61SB-c0mHGL._SX329_BO1,204,203,200_.jpg",
    "read": false
  },
  {
    "id": 3,
    "title": "White Teeth",
    "author": "Zadie Smith",
    "img": "https://images-na.ssl-images-amazon.com/images/I/412BMH3wiIL._SX322_BO1,204,203,200_.jpg",
    "read": false
  },
  {
    "id": 4,
    "author": "Zadie Smith",
    "img": "https://images-na.ssl-images-amazon.com/images/I/41oS2tKs2TL._SX324_BO1,204,203,200_.jpg",
    "title": "On Beauty",
    "read": false
  },
  {
    "id": 5,
    "author": "Zadie Smith",
    "img": "https://images-na.ssl-images-amazon.com/images/I/51Vfwe7%2BJ5L._SX324_BO1,204,203,200_.jpg",
    "title": "Swing Time",
    "read": false
  },
  {
    "id": 6,
    "author": "Zadie Smith",
    "img": "https://images-na.ssl-images-amazon.com/images/I/517BqHtwIKL._SX324_BO1,204,203,200_.jpg",
    "title": "Feel Free",
    "read": false
  },
  {
    "id": 7,
    "author": "Maya Angelou",
    "img": "https://images-na.ssl-images-amazon.com/images/I/41SKsBaGXRL._SX301_BO1,204,203,200_.jpg",
    "title": "I Know Why the Caged Bird Sings",
    "read": false
  },
  {
    "id": 8,
    "author": "Toni Morrison ",
    "img": "https://images-na.ssl-images-amazon.com/images/I/413-xT0izBL._SX322_BO1,204,203,200_.jpg",
    "title": "The Bluest Eye",
    "read": false
  },
  {
    "title": "War And Peace",
    "author": "Leo Tolstoy",
    "img": "https://images-na.ssl-images-amazon.com/images/I/51J1nb00FLL._SX330_BO1,204,203,200_.jpg",
    "id": 9,
    "read": false
  },
  {
    "title": "Pride & Prejudice",
    "author": "Jane Austen ",
    "img": "https://images-na.ssl-images-amazon.com/images/I/514uGV1Y9kL._SX322_BO1,204,203,200_.jpg",
    "id": 13,
    "read": false
  },
  {
    "title": "Green Eggs & Ham",
    "author": "Dr. Seuss",
    "img": "https://images-na.ssl-images-amazon.com/images/I/61akeHxE0OL._SX258_BO1,204,203,200_.jpg",
    "id": 14,
    "read": false
  },
  {
    "title": "The Bible",
    "img": "https://media.beliefnet.com/~/media/photos-with-attribution/faith/001/bible_cover.jpg",
    "author": "The People",
    "read": false,
    "id": 15
  },
  {
    "title": "Lifecycle docs",
    "img": "https://miro.medium.com/max/1099/1*Q1JUFppwxVg8FgzYbZ-eIA.png",
    "author": "React Devs",
    "read": false,
    "id": 16
  }
]

User.destroy_all
Book.destroy_all

@user = User.create(email: "sample@sample.com", password: "test")
@books = @book_data.each do |book|
  Book.create(
    title: book[:title],
    img: book[:img],
    author: book[:author],
    read: false,
    user_id: @user.id
  )
end
